                    ...
                    <table id="table-1" class="table table-striped table-bordered table-hover">
                        ...
                        <c:choose>
                            <c:when test="${r'${empty page.list}'}">
                                <tr>
                                    <td class="center" colspan="16">对不�?,数据为空!</td>
                                </tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach items="${r'${page.list}'}" var="data" >
                                    ...
                                    <#list  fields as field >
                                        ...
                                        <td>
                                            <a class="btn btn-minier btn-success" href="${mappingUrl}/editPage?id=${r'${data.id}'}">
                                                <i class="fa fa-pencil" aria-hidden="true"></i>编辑
                                            </a>
                                            <a class="btn btn-minier btn-danger" href="javascript:void(-1)" onclick="delData(${r'${data.id}'})">
                                                <i class="fa fa-trash-o" aria-hidden="true"></i>删除
                                            </a>
                                            <#--<a class="btn btn-purple btn-minier" href="${mappingUrl}/detailPage?id=${r'${data.id}'}">-->
                                                <#--详情-->
                                            <#--</a>-->
                                        </td>
                                    </tr>
